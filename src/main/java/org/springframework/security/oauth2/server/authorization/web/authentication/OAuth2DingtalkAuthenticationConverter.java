...
import org.springframework.security.oauth2.server.authorization.authentication.OAuth2DingtalkAuthenticationToken;
...
/**
 * 钉钉 dingtalk OAuth2 用于验证授权授予的 {@link OAuth2DingtalkAuthenticationToken}。
 *
 * @author xuxiaowei
 * @since Joe Grandja
 * @since 0.0.1
...
public class OAuth2DingtalkAuthenticationConverter implements AuthenticationConverter {
	...
	public Authentication convert(HttpServletRequest request) {
		...
		if (!OAuth2DingtalkAuthenticationToken.DINGTALK.getValue().equals(grantType)) {
			return null;
		}
		...
		// appid (REQUIRED)
		String appid = parameters.getFirst(OAuth2DingtalkParameterNames.APPID);

		if (!StringUtils.hasText(appid) || parameters.get(OAuth2DingtalkParameterNames.APPID).size() != 1) {
			OAuth2EndpointUtils.throwError(OAuth2ErrorCodes.INVALID_REQUEST, OAuth2DingtalkParameterNames.APPID,
					OAuth2DingtalkEndpointUtils.AUTH_CODE2SESSION_URI);
		}
		...
		parameters.forEach((key, value) -> {
			if (!key.equals(OAuth2ParameterNames.GRANT_TYPE) && !key.equals(OAuth2ParameterNames.CLIENT_ID)
					...
					&& !key.equals(OAuth2DingtalkParameterNames.APPID) && !key.equals(OAuth2ParameterNames.SCOPE)
					&& !OAuth2DingtalkParameterNames.REMOTE_ADDRESS.equals(key)
					&& !OAuth2DingtalkParameterNames.SESSION_ID.equals(key)
					&& !OAuth2DingtalkParameterNames.BINDING.equals(key)) {
				additionalParameters.put(key, value.get(0));
			}
		});

		String remoteAddress = request.getParameter(OAuth2DingtalkParameterNames.REMOTE_ADDRESS);
		String sessionId = request.getParameter(OAuth2DingtalkParameterNames.SESSION_ID);

		return new OAuth2DingtalkAuthenticationToken(clientPrincipal, additionalParameters, appid, code, scope,
